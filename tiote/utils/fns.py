...
def generate_sidebar(request):
    ...
    if request.GET.get('sctn') == 'home':
    ...
    elif request.GET.get('sctn') == 'overview':
        ...
        placeholder = '<h6 class="placeholder">%ss:</h6>' % ABBREVS[request.GET.get('sctn')]
        sfx = "<ul>{0}</ul>".format( ''.join(sfx_list) )
        ret_string = '<h6>quick nav:</h6>\
        <div class="sidebar-item"><img class="icon-databases" /> {0}</div>{1}{2}{3}'.format( 
            db_selection_form, s, placeholder, sfx
        )
